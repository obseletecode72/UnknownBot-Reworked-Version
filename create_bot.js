...
if (isMainThread) {
            ...
            message.json.extra.forEach((extra) => {
                if (extra && extra.clickEvent) {
                    if (unidecode(extra.text) === unidecode(textFromFile)) {
                        ...
                        parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: "<br/><span style='color:green'>Mensagem clicada!</span><br/>" } });
                    }
                }
            });
        }
    ...
    async function minerar(bot, blockType) {
        ...
        if (!blockToMine) {
            parentPort.postMessage({ type: 'electron', action: 'log', data: 'Não conheço o bloco ' + blockType});
            return;
        }
        ...
        if (!block) {
            parentPort.postMessage({ type: 'electron', action: 'log', data: 'Não encontrei nenhum bloco de ' + blockType + 'próximo.'});
            miningState[bot.username] = false;
            return;
        }
    
        parentPort.postMessage({ type: 'electron', action: 'log', data: 'Encontrei um bloco de ' + blockType + '.' + ' Começando a minerar.'});
        ...
        try {
            ...
            clearInterval(checkInterval);
            if (miningState[bot.username]) {
                parentPort.postMessage({ type: 'electron', action: 'log', data: 'Minerado um bloco de ' + blockType + '.'});
                minerar(bot, blockType);
            }
        } catch (err) {
            parentPort.postMessage({ type: 'electron', action: 'error', data: JSON.stringify(err)});
            miningState[bot.username] = false;
            clearInterval(checkInterval);
        }
    }    

    parentPort.postMessage({type: 'thread-asked-for-both'});
    const createBot = async () => {
        ...
        if (proxy && proxy.ip && proxy.port) {
            ...
            if ((proxyType === '4' || proxyType === '5')) {
                if (!user && !password) {
                    parentPort.postMessage({ type: 'electron', action: 'log', data: 'proxy, no pass, no user, type: ' + proxyType });
                    parentPort.postMessage({ type: 'electron', action: 'log', data: 'targeted proxy: ' + proxy.ip + ":" + proxy.port});
                    if (ipPortRegex.test(host)) {
                        ...
                        if (!record) {
                            parentPort.postMessage({ type: 'electron', action: 'error', data: 'SRV Record nao encontrado, bot anulado...'});
                            return;
                        }
                    ...
                    bot = mineflayer.createBot({
                        ...
                        version: version,
                        viewDistance: 'tiny',
                        fakeHost: useFakeHost ? host : record.ip
                    });
                } else if (user && !password) {
                    parentPort.postMessage({ type: 'electron', action: 'log', data: 'proxy, no pass, type: ' + proxyType });
                    parentPort.postMessage({ type: 'electron', action: 'log', data: 'targeted proxy: ' + proxy.ip + ":" + proxy.port});
                    ...
                    parentPort.postMessage({ type: 'electron', action: 'log', data: 'proxy, type: ' + proxyType});
                    parentPort.postMessage({ type: 'electron', action: 'log', data: 'targeted proxy: ' + proxy.ip + ":" + proxy.port});
                ...
                if (!user && !password) {
                    ...
                    bot = mineflayer.createBot({
                        connect: (client) => {
                            ...
                            req.on('error', (err) => {
                                parentPort.postMessage({ type: 'electron', action: 'error', data: err});
                                parentPort.postMessage({ type: 'webcontents', event: 'update-bot-status', data: { bot: username, status: 'disconnected' } });
                                parentPort.postMessage({ type: 'webcontents', event: 'bot-disconnected', data: username });
                            })
                        },
            ...
            parentPort.postMessage({ type: 'electron', action: 'log', data: 'bot without proxy'});
            bot = mineflayer.createBot({ host, username, version: version, viewDistance: 'tiny', fakeHost: useFakeHost ? host : record.ip, auth: 'offline' });
        }
        ...
        fs.access(pluginDir, fs.constants.F_OK, async (err) => {
            if (err) {
                parentPort.postMessage({ type: 'electron', action: 'error', data: 'Diretório ' + pluginDir + ' não existe.'});
                return;
            }

            fs.readdir(pluginDir, async (err, files) => {
                if (err) {
                    parentPort.postMessage({ type: 'electron', action: 'error', data: 'Não foi possível ler o diretório de plugins: ' + err});
                    return;
                }

                if (files.length === 0) {
                    parentPort.postMessage({ type: 'electron', action: 'log', data: 'Nenhum plugin para carregar.' });
                    return;
                }

                for (let file of files) {
                    if (path.extname(file) === '.js') {
                        parentPort.postMessage({ type: 'electron', action: 'log', data: 'Carregando plugin: ' + file});
                        try {
                            ...
                            parentPort.postMessage({ type: 'electron', action: 'error', data: 'Erro ao carregar o plugin: ' + file + ': ' + err});
                        }
                    }
                }
            });
        ...
        bot.on('connect', async () => {
            ...
            parentPort.postMessage({ type: 'webcontents', event: 'bot-connecting', data: username });
            parentPort.postMessage({ type: 'webcontents', event: 'update-bot-status', data: { bot: username, status: 'connecting' } });

            parentPort.postMessage({ type: 'electron', action: 'log', data: `connecting para ${username}` });

            botTimers[username].timer = setTimeout(() => {
                if (botTimers[username] && botTimers[username].isConnecting) {
                    parentPort.postMessage({ type: 'webcontents', event: 'update-bot-status', data: { bot: username, status: 'disconnected' } });
                    parentPort.postMessage({ type: 'electron', action: 'log', data: `status desconectado após timeout para ${username}` });
        ...
        bot.on('login', () => {
            if (botTimers[username] && botTimers[username].isConnecting) {
                ...
                if (!botsConectado.includes(username)) {
                    parentPort.postMessage({ type: 'webcontents', event: 'bot-connected', data: username });
                    parentPort.postMessage({ type: 'webcontents', event: 'update-bot-status', data: { bot: username, status: 'connected' } });
                    ...
                    parentPort.postMessage({ type: 'electron', action: 'log', data: `logged para ${username}` });
                }
            }
        });
            ...
            if (fallcheckbypass) {
                ...
                packet.records.forEach((record) => {
                    ...
                    if (chunk) {
                        ...
                        parentPort.postMessage({ type: 'electron', action: 'error', data: 'Chunk is undefined when adding block.'});
                    }
                });
            }
        });
        ...
        bot._client.on('block_change', (packet) => {
            if (fallcheckbypass) {
                ...
                if (chunk) {
                    ...
                    parentPort.postMessage({ type: 'electron', action: 'error', data: 'Chunk is undefined when changing block.'});
                }
            }
        });        

        bot.on('end', async () => {
            parentPort.postMessage({ type: 'electron', action: 'log', data: `deleted para ${username}` });
            parentPort.postMessage({ type: 'webcontents', event: 'update-bot-status', data: { bot: username, status: 'disconnected' } });
            parentPort.postMessage({ type: 'webcontents', event: 'bot-disconnected', data: username });
        
            parentPort.postMessage({ type: 'bot-end', username: username });
            ...
            if (autoreconnect) {
                parentPort.postMessage({ type: 'electron', action: 'log', data: 'auto reconect ativado - esperando 50ms' });
                await sleep(50); // esperar ficar tudo ok para o autoreconnect
                await sleep(autoreconnectdelay); // esperar delay do autoreconnect
                parentPort.postMessage({ type: 'electron', action: 'log', data: `auto reconect ativado - esperando ${autoreconnectdelay}ms` });
                if (!botsSemAutoReconnect.includes(username)) {
                    parentPort.postMessage({ type: 'electron', action: 'log', data: 'tentando reconect' });
                    parentPort.postMessage({ type: 'ipcMain', event: 'connect-bot', data: { host: host, username: username, version: version, proxy: proxy && proxy.ip && proxy.port ? proxy : null, proxyType: proxyType } });
                }
            }
        ...
        bot._client.on('map', ({ data }) => {
            ...
            if (item && item.name === 'filled_map') {
                ...
                imagem.toBlob(function (erro, blob) {
                    ...
                    parentPort.postMessage({ type: 'electron', action: 'createCaptchaWindow', data: { botusername: bot.username, captchaImage: captchaImages[bot.username] } });
        ...
        bot.on('error', (err) => {
            parentPort.postMessage({ type: 'electron', action: 'log', data: 'bot_on_error: ' + err});
        })
        ...
        bot.on('kicked', (reason) => {
            ...
            parentPort.postMessage({ type: 'electron', action: 'log', data: 'bot_on_kicked_reason_obj: ' + JSON.stringify(reasonObj)});
            ...
            parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: username, message: htmlMessage } });
        
            parentPort.postMessage({ type: 'electron', action: 'log', data: 'bot_on_kicked_html_message: ' + htmlMessage});
        ...
        bot.on('message', (message) => {
            ...
            parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: fullMessage } });
        ...
        bot.on('title', (title) => {
            try {
                ...
                parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: fullTitle } });
            } catch (error) {
                parentPort.postMessage({ type: 'electron', action: 'error', data: 'Error: ' + error });
            }
        });
    ...
    parentPort.on('message', (process_msg_) => {
        if (process_msg_.type === 'send-message' && bot.username) { // precisa do && bot.username, para verificar se o bot e valido ou nao
            const { message } = process_msg_;
            ...
            if (message.startsWith('$')) {
                if (!global.listadecommandos.some(cmd => message.toLowerCase().startsWith(cmd.toLowerCase()))) {
                    parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: "<br/><span style='color:red'>Comando Inexistente</span><br><br/>" + global.Syntax } });
                }
                    ...
                    if (entity) {
                        ...
                        bot.attack(entity)
                        parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: `<br/><span style='color:green'>Entidade clicada!</span><br/>` } });
                    } else {
                        parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: `<br/><span style='color:red'>Nenhuma entidade proxima para clicar!</span><br/>` } });
                    }
                }
                    ...
                    if (isNaN(slotNumber) || slotNumber < 0 || slotNumber > 8) {
                        parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: `<br/><span style='color:red'>Valor invalido!</span><br/>` } });
                    }
                    else {
                        bot.setQuickBarSlot(slotNumber);
                        parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: `<br/><span style='color:green'>Slot da Hotbar setado para ${slotNumber}!</span><br/>` } });
                    }
                }
                else if (message.toLowerCase() == "$listslots") {
                    if (bot.currentWindow) {
                        parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: `<br/><span style='color:green'>A janela atual tem ${bot.currentWindow.slots.length} slots!</span><br/>` } });
                    } else {
                        parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: `<br/><span style='color:red'>Nao ha janela aberta</span><br/>` } });
                    }
                }
                else if (message.toLowerCase() == "$holditem") {
                    bot.activateItem();
                    parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: `<br/><span style='color:green'>Item clicado!</span><br/>` } });

                    if (bot.usingHeldItem) {
                        parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: `<br/><span style='color:green'>Item usavel, parando de clicar!</span><br/>` } });
                        bot.deactivateItem();
                    }
                }
                    ...
                    if (isNaN(slotNumber)) {
                        ...
                        (async () => {
                            ...
                            if (item) {
                                ...
                                parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: `<br/><span style='color:green'>Dropando no slot do inventario ${slotNumber}!</span><br/>` } });
                            }
                        })();
                    }
                    else {
                        bot.clickWindow(slotNumber, 0, 0);
                        parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: `<br/><span style='color:green'>Clicando no slot do inventario ${slotNumber}!</span><br/>` } })
                    }
                }
                else if (message.toLowerCase().startsWith("$dropall")) {
                    (async () => {
                        parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: `<br/><span style='color:green'>Dropando todos itens!</span><br/>` } })
                        ...
                        parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: `<br/><span style='color:green'>Todos itens dropados!</span><br/>` } })
                    })();
                }
                else if (message.toLowerCase().startsWith("$inventoryinterface")) {
                    let args = message.slice("$inventoryinterface".length).trim();
                    switch (args) {
                        case '':
                            (async () => {
                                parentPort.postMessage({ type: 'closeeverywindowinventory' });
                                await sleep(200);
                                let options = {
                                    port: 9531,
                                    startOnLoad: false
                                ...
                                if (!bot.webInventory.isRunning) {
                                    ...
                                    parentPort.postMessage({ type: 'electron', action: 'createWebInventoryWindow', data: { bot: bot.username } });
                                    isWindowClosing = false;
                                    parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: "<br/><span style='color:green'>Interface sendo aberta!</span><br><br/>" } });
                                }
                            })();
                            break;
                        case 'close':
                    ...
                    (async () => {
                        ...
                        if (miningState[bot.username]) {
                            parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: `<br/><span style='color:green'>Miner ativado!</span><br/>` } })
                        } else {
                            parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: `<br/><span style='color:red'>Miner Desativado!</span><br/>` } })
                        }
                    ...
                    switch (command) {
                        case 'parkour':
                            ...
                            parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: `<br/><span style='color:orange'>Modo Parkour ${parkourStatus}!</span><br/>` } });
                            break;
            
                        case 'placeblock':
                            globalBlockPlacingAllowed = !globalBlockPlacingAllowed;
                            const blockPlacingStatus = globalBlockPlacingAllowed ? 'permitida' : 'proibida';
                            parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: `<br/><span style='color:orange'>Colocação de blocos ${blockPlacingStatus}!</span><br/>` } });
                            break;
                        ...
                        default:
                            if (args.length === 7) {
                                ...
                                parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: `<br/><span style='color:red'>Comando inválido. Use $miner [parkour|placeblock] ou $miner x1 y1 z1 x2 y2 z2</span><br/>` } });
                            }
                    }
                }
                    ...
                    if (args.length === 1) {
                        ...
                        if (killAuraActive[bot.username]) {
                            parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: "<br/><span style='color:green'>KillAura ativado</span><br/>" } });
                            ...
                            parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: "<br/><span style='color:red'>KillAura desativado</span><br/>" } });
                        ...
                        switch (command) {
                            case 'timems':
                                ...
                                parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: `<br/><span style='color:green'>Delay do KillAura definido para ${killAuraDelay}ms</span><br/>` } });
                                break;
                
                            case 'distance':
                                DistanceReach = parseInt(args[2]);
                                if (DistanceReach <= 0 || DistanceReach > 6 || isNaN(DistanceReach)) {
                                    parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: `<br/><span style='color:red'>Valor inválido! Distância setada para 3m</span><br/>` } });
                                    DistanceReach = 3;
                                } else {
                                    parentPort.postMessage({ type: 'electron', action: 'log', data: DistanceReach});
                                    parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: `<br/><span style='color:green'>Distância do KillAura definida para ${DistanceReach}m</span><br/>` } });
                                }
                                break;
                
                            default:
                                parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: `<br/><span style='color:red'>Comando inválido. Use $killaura, $killaura timems [valor], ou $killaura distance [valor]</span><br/>` } });
                        }
                    } else {
                        parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: `<br/><span style='color:red'>Comando inválido. Use $killaura, $killaura timems [valor], ou $killaura distance [valor]</span><br/>` } });
                    }
                    ...
                    if (isMoving[bot.username]) {
                        bot.pathfinder.setGoal(null);
                        parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: "<br/><span style='color:red'>Parando o trajeto ate as coordenadas</span><br/>" } })
                        ...
                        parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: "<br/><span style='color:green'>Indo ate as coordenadas fornecidas...</span><br/>" } })
                        isMoving[bot.username] = true;

                        bot.on('goal_reached', function () {
                            if (isMoving[bot.username]) {
                                parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: `<br/><span style='color:green'>Cheguei nas coordenadas ${x}, ${y}, ${z}</span><br/>` } })
                                isMoving[bot.username] = false;
                            }
                        });
                    }
                    ...
                    if (directions == '') {
                        parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: "<br/><span style='color:red'>Argumentos invalidos!</span><br/>" } });
                    }
                    else {
                        parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: "<br/><span style='color:green'>Movendo ate as direções \"" + directions + "\" com a duração de " + duration + " ticks</span><br/>" } });
                    ...
                    if (IsSneaking[bot.username]) {
                        parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: "<br/><span style='color:green'>Shift Ativado</span><br/>" } })
                        bot.setControlState('sneak', true);
                    }
                    else {
                        bot.setControlState('sneak', false);
                        parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: "<br/><span style='color:red'>Shift desativado</span><br/>" } })
                    }
                }
        ...
        if(process_msg_.type === 'remove-bot')
        {
            const { botUsername } = process_msg_;
            botsSemAutoReconnect.push(botUsername);
            bot.end();
            process.exit(999); // ok, nao tem por que continuar ja que o bot simplesmente foi removido
        }
        if(process_msg_.type === 'reco-bot')
        {
            const { host, botUsername, version, proxy, proxyType } = process_msg_;
            const botExists = bot.username === botUsername;

            if (!botExists) {
                parentPort.postMessage({ type: 'ipcMain', event: 'connect-bot', data: { host: host, username: botUsername, version: version, proxy: proxy && proxy.ip && proxy.port ? proxy : null, proxyType: proxyType } });
            } else {
                botsSemAutoReconnect.push(botUsername);
                bot.end();
                parentPort.postMessage({ type: 'ipcMain', event: 'connect-bot', data: { host: host, username: botUsername, version: version, proxy: proxy && proxy.ip && proxy.port ? proxy : null, proxyType: proxyType } });
        ...
        if(process_msg_.type === 'captcha-input-janela1')
        {
            const { captchaInput } = process_msg_;
            bot.chat(captchaInput);
            parentPort.postMessage({ type: 'webcontents', event: 'bot-message', data: { bot: bot.username, message: "<br/><span style='color:green'>Captcha enviado!</span><br/>" } });
            parentPort.postMessage({ type: 'closewindowcaptcha', username: bot.username });
        }
        if (process_msg_.type === 'form-data-changed-reconnect') {
            autoreconnect = process_msg_.data.autoReconnect;
            autoreconnectdelay = parseInt(process_msg_.data.delay);
        }
        
        if (process_msg_.type === 'form-data-changed-fallcheck') {
            fallcheckbypass = process_msg_.data.fallCheck;
        }
        if(process_msg_.type === 'closewindowinventoryhardcoded')
        {
            if (bot.webInventory && bot.webInventory.isRunning) {
                bot.webInventory.stop();
                parentPort.postMessage({ type: 'electron', action: 'closewindowinventory' });
            }
        } 
        if(process_msg_.type === 'botsConectadoUpdate')
        {
            botsConectado = process_msg_.data;
        } 
    });
}
