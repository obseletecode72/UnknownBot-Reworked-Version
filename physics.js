...
function inject(bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  function tickPhysics(now) {
    if (bot.blockAt(bot.entity.position) == null && bot.vehicle == null) return // check if chunk is unloaded
    ...
    if (shouldUsePhysics || bot.physicsEnabled) {
      updatePosition(now)
    }
  }
  ...
  function sendPacketSteerVehicle(sideways, forward, jump) {
    // sends data, no logic
    const packetstruct = {};
    packetstruct.sideways = sideways
    packetstruct.forward = forward
    packetstruct.jump = jump
    bot._client.write('steer_vehicle', packetstruct)
  }
  ...
  function updatePosition(now) {
    ...
    if (positionUpdated && lookUpdated && bot.vehicle == null) {
      sendPacketPositionAndLook(position, yaw, pitch, onGround)
      lastSent.time = now // only reset if positionUpdated is true
    }
    else if (positionUpdated && bot.vehicle == null) {
      sendPacketPosition(position, onGround)
      lastSent.time = now // only reset if positionUpdated is true
    } else if (lookUpdated && bot.vehicle == null) {
      sendPacketLook(yaw, pitch, onGround)
    } else if ((positionUpdateSentEveryTick || onGround !== lastSent.onGround) && bot.vehicle == null) {
    ...
    if (bot.vehicle != null) {
      sendPacketLook(yaw, pitch, lastSent.onGround) // aqui
      sendPacketSteerVehicle(0, 0, 0);
      lastSent.time = now // only reset if positionUpdated is true
    }
  ...
  bot._client.on('position', (packet) => {
    if (bot.abilities.flags === 6 && packetscount > 0) return;
  ...
  bot.on('mount', () => { shouldUsePhysics = true })
  bot.on('respawn', () => {
